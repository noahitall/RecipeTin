...
export function loadStaticData(realm, user) {
  ...
  // TODO only only objects if matching id doesn't exist
  realm.write(() => {

    // Load objects in order of references
    // Categories first
    categories.forEach((category) => {
      //check if a category with the same id exists in the realm
      const existingCategory = realm.objects("Category").filtered("categoryId = $0", category.id)[0];
      if (!existingCategory) {
        new Category(realm, category.name, category.photo_url, category.id, user?.id);
      }
    });
    ...
    ingredients.forEach((ingredient) => {
      //check if an ingredient with the same id exists in the realm
      const existingIngredient = realm.objects("Ingredient").filtered("ingredientId = $0", ingredient.ingredientId)[0];
  ...
  realm.write(() => {
    ...
    stepIngredients.forEach((stepIngredient) => {
      //check if a stepIngredient with the same id exists in the realm
      const existingStepIngredient = realm.objects("StepIngredient").filtered("stepIngredientId = $0", stepIngredient.stepIngredientId)[0];
      if (!existingStepIngredient) {
  ...
  realm.write(() => {
    // Step
    steps.forEach((step) => {
      //check if a step with the same id exists in the realm
      const existingStep = realm.objects("Step").filtered("stepId = $0", step.stepId)[0];
      if (!existingStep) {
        // Map the list of stepIngredients stepIngredientIds to the actual stepIngredient objects
        const stepIngredients = step.stepIngredients.map((stepIngredientId) => getStepIngredientById(realm, stepIngredientId))
        console.log("stepIngredients", JSON.stringify(stepIngredients));
  ...
  realm.write(() => {
    ...
    recipes.forEach((recipe) => {
      //check if a recipe with the same id exists in the realm
      const existingRecipe = realm.objects("Recipe").filtered("recipeId = $0", recipe.recipeId)[0];
      if (!existingRecipe) {
        ...
        console.log("recipeStepIngredients", JSON.stringify(recipeStepIngredients));
  ...
  console.log("Objects in realm");
  console.log("Found: " + realm.objects("Category").length + " categories");
  console.log("Found: " + realm.objects("Ingredient").length + " ingredients");
  console.log("Found: " + realm.objects("StepIngredient").length + " stepIngredients");
  console.log("Found: " + realm.objects("Step").length + " steps");
  console.log("Found: " + realm.objects("Recipe").length + " recipes");
